...
void ct(z a[],z *seg,z low,z high ,z pos) 
{
    ...
    seg[pos]=min(seg[2*pos+1],seg[2*pos+2]); // minimum of left and right child will become parent 
}
//---------------------------------------------------------------------------------------------------------------------------------------
// function to answer range min queries 
z range(z seg[],z ql,z qh,z low,z high,z pos)
{
    ...
    if(low>qh || high <ql) 
    {
        return INT_MAX; // just return INT_MAX because non overlapped nodes can not contribute in min
    }
    // PARTIAL OVERLAP
    z mid =(low+high)/2;
    ...
    return min(range(seg,ql,qh,low,mid,2*pos+1),range(seg,ql,qh,mid+1,high,2*pos+2));

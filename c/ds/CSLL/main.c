...
int main(void) {
    ...
    /* Declare variables head, tail and newnode of type nodeptr_t */
...
//------------------------------------- Circular Singly Linked Lists----------------------------------------------
    ...
    if (bptr2==NULL) perror("Error opening file: ");
        ...
        for(i=0;i<N;i++){
            ...
            newnode = CreateNodeCSLL(rand_nbrs[i], &head);
            InsertListCSLL(&newnode,&head);

            //display length & contents of list with new value inserted
            length_var =LengthCSLL(&head);
    ...
    if (bptr4==NULL) perror("Error opening file: ");
        ...
        for(i=0;i<N;i++){
            ...
            DeleteNodeCSLL(rand_nbrs[i],&head);

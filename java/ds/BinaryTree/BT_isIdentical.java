...
public class BT_isIdentical {

	static Scanner scn = new Scanner(System.in);

	public static void main(String[] args) {

		BT_isIdentical bts = new BT_isIdentical();
		BinaryTree bt = bts.new BinaryTree();
		// 10 true 20 true 40 false false true 50 false false true 30 true 60 false
		// false false **Give this Input to create the tree**

		BinaryTree bt2 = bts.new BinaryTree(); // Create A Second Binary Tree

		System.out.println(bt.isIdentical(bt));
	...
	private class BinaryTree {
		...
		private boolean isIdentical(Node node, Node onode) {
			...
			if (node != null && onode != null) {
				return (node.data == onode.data && isIdentical(node.left, onode.left)
						&& isIdentical(node.right, onode.right));
			}
			return false;
